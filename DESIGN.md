...
- **Phone (under 48em).** Both frames are **square and edge to edge**, 390×390 at 390px — the treatment `.keeper__portrait` has had since the page was built, and the one thing on that screen the client never called small.
  - `.stage__sign` drops to `grid-row: 1`, so the drawn tree keeps the corridor beside the *text* only. The spine's node follows it and stays in the open.
  - **The drawn line now passes behind the moment photos**, which is a real change to a client decision (the line was never to be switched off on the phone). It is not switched off: it enters under the sprayed edge and comes out below. The corridor was eating 78px of a 390px screen, about a quarter of the width, to hold a decoration in front of the reason people open the page. **Reverting is two declarations:** drop `grid-column` and `margin-inline` from `.stage__frame` in the phone band and the photo returns to its column.
  - `.reel__track` loses its side padding in that band, or the first slide can never reach the centre (scroll cannot go below zero) and sits pushed right. Every slide was tested, first to last: each lands exactly centred.
