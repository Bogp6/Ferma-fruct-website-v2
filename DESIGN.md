...
- `.fruit` — one row of four photographic plates, in picking order. Rebuilt 2026-08-15 from a reference the client sent (webflow-path-two.webflow.io). The plate is dark and desaturated at rest and comes up to full colour under the pointer, with a ring that trails the cursor and says "Vezi". Went through two-across-large first; client wanted it smaller and the row tighter.
  ...
  - Grid is `repeat(4, minmax(0, 1fr))` with a tight `--space-s` gap, declared not auto-fit: auto-fit would strand one on a second line. The gap is tight on purpose — four plates close together read as one band.
  - Nothing sits under the plate. The sentence about the fruit is on the card, above the name, and only where a pointer can hover: `@media (hover: hover)` hides it, so a phone — which never gets a hover — keeps it visible. Do not invert that query.
  - `.fruit__note` is **absolutely positioned and out of the plate's flow**, with `inset-block-end: 100%`. In flow it is two lines on one card and three on another, and that difference walks the four names up and down out of line with each other. Anything that puts it back in flow breaks that alignment.
  - `.fruit__plate::before` is a second scrim that fades in only while the sentence shows, so small text never lands on the bright part of a photograph at the moment the wash is dropping to 0.14. It is not a darkening of the hover state; the wash above it is doing far more in the other direction.
  - Photos are 3:4 portrait centre crops at `*-p320.webp` / `*-p640.webp`. Portrait, not the landscape crop the two-across version used: the card is portrait now, and it also means the 640px-wide cireșe source is finally enough for a 2x file. The old `*-280/-560.webp` files are unused.
