...
- `.grove` — one block per fruit, four of them, in picking order. Replaced the old row of four `.fruit` plates; that component and its cursor-trailing ring are gone from CSS/JS, recoverable from commit `121458f`.
  ...
  - **Photo silhouette is a watercolour wash** (webp alpha mask, `mask-image: var(--blob)`, files `img/blob-{a1,a2,b1,b2}.webp`, `-webkit-mask-*` for Safari) — not `radial-gradient` (too clean an oval) and not `feTurbulence` (paints every frame). `mask-size: 100% 100%` stretches to the box, so `blob-a*` is square and `blob-b*` is 4:3 to match each frame's aspect ratio — **change a photo's ratio and the mask must be regenerated.** Set by `:nth-child` so no two of the four blocks share an outline.
    ...
    - **`blob-{a5,a6,a7}.webp` (2026-08-23), the phone masks on despre.** 900×900 (1:1), same `blob-e` recipe and the same `REACH = 0.935` rescaling, seeds 317 / 88 / 445. Fill 59.0 / 61.3 / 60.0%, 0.00% semi-transparent, every edge 0% solid — measured, in family with `a3` (60.4%).
      - They exist because under 48em both `.stage__photo` and `.reel__media` turn square. A 16:9 `blob-e` stretched over a 1:1 box would comb the granulation into vertical dashes. Three of them, not two, so the carousel's `3n` cycle never repeats a silhouette inside one screen.
      - `a3`/`a4` were left alone on index rather than reused here, so a change to the phone masks cannot reach the landing page.
      - The generator is a standalone numpy/PIL script kept outside the repo; the recipe above is the whole of it, and these seeds reproduce these three files exactly.
...
### The photos were too small on phone and tablet (2026-08-23)

Client's verdict, three rounds of it, on the live site. The photos on `despre.html` read as small at every narrow width. What was measured before the fix: at 390px the moment photo was 312×195 in a 390 window, and the carousel 343×193. Both already ran to the glass on the right, so **width was spent and the only lever left was the frame's ratio.**

- **Tablet (48–64em).** The moment photo's `max-width: 30rem` cap is gone: it holds the whole column and ends on the same margin as the text above it. The carousel slide is `calc(100vw - var(--gutter) * 2)`, the page's own content width, instead of `clamp(16rem, 88vw, 46rem)` — the `46rem` cap was biting from 836px up and left 143px of empty paper on each side at 1023px. Both frames are 16:10 there.
  - The eight drawings that lived in the strip of paper beside the moment photos (`.motif--roots-leaf-d` … `-j`, `.motif--roots-apple`) are switched off on tablet. That strip no longer exists, and at `z-index: -1` they would have vanished under the photo anyway. Ten drawings still show in that section.
- **Phone (under 48em).** Both frames are **square**. The carousel is square *and* edge to edge, 390×390 at 390px — the treatment `.keeper__portrait` has had since the page was built. `.reel__track` loses its side padding in that band, or the first slide can never reach the centre (scroll cannot go below zero) and sits pushed right. Every slide was tested, first to last: each lands exactly centred.
  - The moment photo is square **inside its own column**: 312×312 at 390px, up from 312×195. It keeps the bleed to the right glass it always had.
  - **The moment photo was widened across the whole screen once, and the client rejected it.** That version put `grid-column: 1 / -1` plus a negative `margin-inline` on `.stage__frame` and moved `.stage__sign` to `grid-row: 1`, which freed the corridor's 78px — a quarter of a 390px screen. It cost too much: the drawn tree, stranded beside the text, read as small and out of proportion against a full-width photo, and the drawn line ran in under the photographs. **Do not re-try it.** On this page the corridor is not spare paper; the tree and the line are the section. Width on the phone is fixed, and the only lever on photo size is the frame's ratio.
- **Square photo files for the phone, via `<picture>`.** A square box cut from the 1200×675 moment files leaves 675×675, which is under 2x on a 390 box. The originals were found by matching the shipped crops against every large file in `img/` — `about-start` ← `before-harvest-about.webp` (2492×1652), `about-rows` ← `row-1-net.webp` (1600×1200), `about-today` ← `row-2.webp` (1600×1200), each an exact full-width crop at a vertical offset (scores 1.000, 0.999, 0.999). The square is cut around the same centre, so the framing the client approved is kept: `*-a800.webp` and `*-a1200.webp`.
  - `about-rows-a1200.webp` is **363KB** — the anti-hail net is fine mesh and does not compress; q68 only saved 25KB. Flagged for the speed pass, not fixed by dropping quality.
  - The carousel needs no new files: a square cut from its 1440×810 files is 810×810 on a 390 box, which is 2.08x.
  - The `<source>` breakpoint is written to match `despre.css` exactly. **Change one and you must change the other**, or the phone gets a square file in a 16:10 box.
