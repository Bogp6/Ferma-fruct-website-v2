...
- `.grove` — one block per fruit, four of them, in picking order. Replaced the row of four `.fruit` plates on 2026-08-15 at the client's call: the fruits are described on this page now, so there is no fruit page and nothing links out. The old `.fruit` component and its cursor-trailing ring are gone from CSS and JS; recover from commit `121458f` if ever wanted back.
  ...
  - The leaf silhouette is `clip-path: inset(0 round …)` on `.grove__frame` — no `border-radius`, no `overflow`. One element does the shape and the arrival wipe. Consequence: a `box-shadow` would be clipped off, so the photos sit flat on the cream. That is the reference's look, not an oversight.
  - `--leaf-round` is a **percentage** (34%), not a length. A percentage corner is elliptical, so it follows the box instead of cutting a quarter-circle out of it, and the two photos of a block come out of the same shape despite different aspects. Under ~25% it stops reading as a leaf.
  - The two frames take opposite diagonals, and `.grove--flip` swaps those too.
  ...
  - A twelve-month season chart was built here on 2026-08-15 and deleted the same day, client's call. Months stay in words in `.grove__season`, and they are still placeholders — see Still open.
